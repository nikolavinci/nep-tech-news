  ...
  return (
    <div className="container mx-auto px-4 py-8 max-w-6xl">
      <header className="mb-10 border-b-2 border-primary pb-4 inline-block">
        <h1 className="text-4xl font-bold">{isEn ? titleEn : titleNp}</h1>
      </header>

      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
        {[1, 2, 3, 4, 5, 6].map((i) => (
          <Link href={`/${lang}/article/${category}-story-${i}`} key={i} className="group flex flex-col gap-4">
            ...
            <div>
              <h2 className="text-xl font-bold leading-tight group-hover:text-primary transition-colors line-clamp-3 mb-2">
                {isEn 
                  ? `Detailed coverage and in-depth analysis on the latest ${titleEn} trends part ${i}` 
              ...
              <p className="mt-2 text-muted-foreground line-clamp-2">
                {isEn 
                  ? 'This is a brief excerpt from the article that gives the reader a little more context about the headline.' 
